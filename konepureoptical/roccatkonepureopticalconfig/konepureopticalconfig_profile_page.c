...
#define KONEPUREOPTICALCONFIG_PROFILE_PAGE_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREOPTICALCONFIG_PROFILE_PAGE_TYPE, KonepureopticalconfigProfilePageClass))
#define IS_KONEPUREOPTICALCONFIG_PROFILE_PAGE_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREOPTICALCONFIG_PROFILE_PAGE_TYPE))
#define KONEPUREOPTICALCONFIG_PROFILE_PAGE_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREOPTICALCONFIG_PROFILE_PAGE_TYPE, KonepureopticalconfigProfilePagePrivate))

typedef struct _KonepureopticalconfigProfilePageClass KonepureopticalconfigProfilePageClass;
typedef struct _KonepureopticalconfigProfilePagePrivate KonepureopticalconfigProfilePagePrivate;

struct _KonepureopticalconfigProfilePage {
	RoccatProfilePage parent;
	KonepureopticalconfigProfilePagePrivate *priv;
};

struct _KonepureopticalconfigProfilePageClass {
	RoccatProfilePageClass parent_class;
};

struct _KonepureopticalconfigProfilePagePrivate {
	KoneplusRmp *rmp;
	KonepureButtonsFrame *buttons;
	RoccatGamefileSelector *gamefiles;
	RoccatPollingRateFrame *polling_rate;
	KonepureopticalconfigCpiSelector *cpi;
	RoccatSensitivityFrame *sensitivity;
	KonepureLightEffectsFrame *light_effects;
	KonepureColorFrame *color;
};

G_DEFINE_TYPE(KonepureopticalconfigProfilePage, konepureopticalconfig_profile_page, ROCCAT_PROFILE_PAGE_TYPE);

GtkWidget *konepureopticalconfig_profile_page_new(void) {
	return GTK_WIDGET(g_object_new(KONEPUREOPTICALCONFIG_PROFILE_PAGE_TYPE, NULL));
}

static void set_from_rmp(KonepureopticalconfigProfilePage *profile_page) {
	KonepureopticalconfigProfilePagePrivate *priv = profile_page->priv;
	guint i;
	...
	konepureopticalconfig_cpi_selector_set_from_rmp(priv->cpi, priv->rmp);
...
static void update_rmp(KonepureopticalconfigProfilePage *profile_page, KoneplusRmp *rmp) {
	KonepureopticalconfigProfilePagePrivate *priv = profile_page->priv;
	gchar *profile_name;
	guint i;
	...
	konepureopticalconfig_cpi_selector_update_rmp(priv->cpi, rmp);
...
static void reset_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	set_from_rmp(KONEPUREOPTICALCONFIG_PROFILE_PAGE(profile_page));
}

static void konepureopticalconfig_profile_page_init(KonepureopticalconfigProfilePage *profile_page) {
	KonepureopticalconfigProfilePagePrivate *priv = KONEPUREOPTICALCONFIG_PROFILE_PAGE_GET_PRIVATE(profile_page);
	GtkWidget *content;
	GtkWidget *left;
	GtkWidget *light;
	GtkWidget *right;
	...
	priv->cpi = KONEPUREOPTICALCONFIG_CPI_SELECTOR(konepureopticalconfig_cpi_selector_new());
...
static void konepureopticalconfig_profile_page_class_init(KonepureopticalconfigProfilePageClass *klass) {
	g_type_class_add_private(klass, sizeof(KonepureopticalconfigProfilePagePrivate));
}
