...
GType konepureoptical_eventhandler_get_type(void);

#define KONEPUREOPTICAL_EVENTHANDLER_TYPE (konepureoptical_eventhandler_get_type())
#define KONEPUREOPTICAL_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), KONEPUREOPTICAL_EVENTHANDLER_TYPE, KonepureopticalEventhandler))
#define IS_KONEPUREOPTICAL_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), KONEPUREOPTICAL_EVENTHANDLER_TYPE))
#define KONEPUREOPTICAL_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREOPTICAL_EVENTHANDLER_TYPE, KonepureopticalEventhandlerClass))
#define IS_KONEPUREOPTICAL_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREOPTICAL_EVENTHANDLER_TYPE))
#define KONEPUREOPTICAL_EVENTHANDLER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREOPTICAL_EVENTHANDLER_TYPE, KonepureopticalEventhandlerPrivate))

typedef struct _KonepureopticalEventhandler KonepureopticalEventhandler;
typedef struct _KonepureopticalEventhandlerClass KonepureopticalEventhandlerClass;
typedef struct _KonepureopticalEventhandlerPrivate KonepureopticalEventhandlerPrivate;

struct _KonepureopticalEventhandler {
	GObject parent;
	KonepureopticalEventhandlerPrivate *priv;
};

struct _KonepureopticalEventhandlerClass {
	GObjectClass parent_class;
};

struct _KonepureopticalEventhandlerPrivate {
	RoccatEventhandlerHost *host;
	KonepureopticalDBusServer *dbus_server;
	...
	KoneplusGfx *gfx;
	KonepureopticalEventhandlerChannel *channel;
...
static void profile_changed(KonepureopticalEventhandler *eventhandler, guint profile_number) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	konepureoptical_dbus_server_emit_profile_changed(priv->dbus_server, profile_number);
}

static gboolean set_profile(KonepureopticalEventhandler *eventhandler, guint profile_number) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static void window_changed_cb(RoccatEventhandlerHost *host, gchar const *title, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static gboolean talk_easyshift(KonepureopticalEventhandler *eventhandler, gboolean state) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
...
static gboolean talk_easyshift_lock(KonepureopticalEventhandler *eventhandler, gboolean state) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talk_easyaim(KonepureopticalEventhandler *eventhandler, guchar state) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_set_led_rgb(KonepureopticalEventhandler *eventhandler, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_restore_led_rgb(KonepureopticalEventhandler *eventhandler) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static guint konepureoptical_eventhandler_get_talk_device(RoccatEventhandlerPlugin *self) {
	KonepureopticalEventhandlerPrivate *priv = KONEPUREOPTICAL_EVENTHANDLER(self)->priv;
...
static void konepureoptical_eventhandler_talk_easyshift(RoccatEventhandlerPlugin *self, gboolean state) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	talk_easyshift(eventhandler, state);
}

static void konepureoptical_eventhandler_talk_easyshift_lock(RoccatEventhandlerPlugin *self, gboolean state) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	talk_easyshift_lock(eventhandler, state);
}

static void konepureoptical_eventhandler_talk_easyaim(RoccatEventhandlerPlugin *self, guchar state) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	talk_easyaim(eventhandler, state);
}

static void konepureoptical_eventhandler_talkfx_set_led_rgb(RoccatEventhandlerPlugin *self, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void konepureoptical_eventhandler_talkfx_restore_led_rgb(RoccatEventhandlerPlugin *self) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	talkfx_restore_led_rgb(eventhandler);
}

static void talk_easyshift_cb(KonepureopticalDBusServer *server, guchar state, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	talk_easyshift(eventhandler, state);
}

static void talk_easyshift_lock_cb(KonepureopticalDBusServer *server, guchar state, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	talk_easyshift_lock(eventhandler, state);
}

static void talk_easyaim_cb(KonepureopticalDBusServer *server, guchar state, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	talk_easyaim(eventhandler, state);
}

static void talkfx_set_led_rgb_cb(KonepureopticalDBusServer *server, guint effect, guint ambient_color, guint event_color, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void talkfx_restore_led_rgb_cb(KonepureopticalDBusServer *server, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	talkfx_restore_led_rgb(eventhandler);
}

static void gfx_set_led_rgb_cb(KonepureopticalDBusServer *server, guint index, guint color, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	koneplus_gfx_set_color(eventhandler->priv->gfx, index, color);
}

static void gfx_get_led_rgb_cb(KonepureopticalDBusServer *server, guint index, guint *color, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	*color = koneplus_gfx_get_color(eventhandler->priv->gfx, index);
}

static void gfx_update_cb(KonepureopticalDBusServer *server, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
...
static void open_gui_cb(gpointer source, gpointer user_data) {
	gchar *path;
	path = g_build_path("/", INSTALL_PREFIX, "bin", "roccatkonepureopticalconfig", (gchar const *)NULL);
	roccat_double_fork(path);
	g_free(path);
}

static void profile_data_changed_outside_cb(KonepureopticalDBusServer *server, guchar number, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->rmp[profile_index] = konepureoptical_rmp_load_actual(profile_index);
}

static void configuration_reload(KonepureopticalEventhandler *eventhandler) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	if (priv->config)
...
static void configuration_changed_outside_cb(KonepureopticalDBusServer *server, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	configuration_reload(eventhandler);
}

static guint cpi_values[KONEPUREOPTICAL_CPI_VALUES_NUM] = { 400, 800, 1200, 1600, 2000, 3200, 4000 };

static void cpi_changed_cb(KonepureopticalEventhandlerChannel *channel, guchar cpi_index, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint cpi_value;
	
	cpi_value = cpi_values[koneplus_rmp_get_cpi_level_x(priv->rmp[priv->actual_profile_index], cpi_index) - 1];
...
static void quicklaunch_cb(KonepureopticalEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void sensitivity_changed_cb(KonepureopticalEventhandlerChannel *channel, guchar x, guchar y, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void talk_cb(KonepureopticalEventhandlerChannel *channel, guchar key_index, guchar event, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_start_cb(KonepureopticalEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_stop_cb(KonepureopticalEventhandlerChannel *channel, gpointer user_data) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(user_data);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static gboolean profile_data_load(KonepureopticalEventhandler *eventhandler, GError **error) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint i;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		priv->rmp[i] = konepureoptical_rmp_load(priv->device, i, error);
		if (priv->rmp[i] == NULL)
			return FALSE;
	}
	return TRUE;
...
static void device_add_cb(RoccatDeviceScannerInterface *interface, RoccatDevice *device, gpointer user_data) {
	...
	if (!priv->profile_note)
		priv->profile_note = roccat_notification_profile_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREOPTICAL_DEVICE_NAME);
	if (!priv->cpi_note)
		priv->cpi_note = roccat_notification_cpi_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREOPTICAL_DEVICE_NAME);
	if (!priv->sensitivity_note)
		priv->sensitivity_note = roccat_notification_sensitivity_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREOPTICAL_DEVICE_NAME);
	if (!priv->timer_note)
		priv->timer_note = roccat_notification_timer_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREOPTICAL_DEVICE_NAME);
	...
	priv->dbus_server = konepureoptical_dbus_server_new();
	...
	konepureoptical_dbus_server_connect(priv->dbus_server);
	...
	konepureoptical_eventhandler_channel_start(priv->channel, priv->device, &local_error);
	if (local_error) {
...
static void stop(KonepureopticalEventhandler *eventhandler) {
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint i;
	...
	konepureoptical_eventhandler_channel_stop(priv->channel);
...
static void konepureoptical_eventhandler_start(RoccatEventhandlerPlugin *self, RoccatEventhandlerHost *host) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->channel = konepureoptical_eventhandler_channel_new();
	...
	priv->device_scanner = konepureoptical_device_scanner_new();
...
static void konepureoptical_eventhandler_stop(RoccatEventhandlerPlugin *self) {
	KonepureopticalEventhandler *eventhandler = KONEPUREOPTICAL_EVENTHANDLER(self);
	KonepureopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void konepureoptical_roccat_eventhandler_interface_init(RoccatEventhandlerPluginInterface *iface) {
	iface->start = konepureoptical_eventhandler_start;
	iface->stop = konepureoptical_eventhandler_stop;
	iface->get_talk_device = konepureoptical_eventhandler_get_talk_device;
	iface->talk_easyshift = konepureoptical_eventhandler_talk_easyshift;
	iface->talk_easyshift_lock = konepureoptical_eventhandler_talk_easyshift_lock;
	iface->talk_easyaim = konepureoptical_eventhandler_talk_easyaim;
	iface->talkfx_set_led_rgb = konepureoptical_eventhandler_talkfx_set_led_rgb;
	iface->talkfx_restore_led_rgb = konepureoptical_eventhandler_talkfx_restore_led_rgb;
...
G_DEFINE_TYPE_WITH_CODE(KonepureopticalEventhandler, konepureoptical_eventhandler, G_TYPE_OBJECT,
		G_IMPLEMENT_INTERFACE(ROCCAT_EVENTHANDLER_PLUGIN_TYPE, konepureoptical_roccat_eventhandler_interface_init));

static void konepureoptical_eventhandler_init(KonepureopticalEventhandler *eventhandler) {
	eventhandler->priv = KONEPUREOPTICAL_EVENTHANDLER_GET_PRIVATE(eventhandler);
}

static void dispose(GObject *object) {
	konepureoptical_eventhandler_stop(ROCCAT_EVENTHANDLER_PLUGIN(object));

	G_OBJECT_CLASS(konepureoptical_eventhandler_parent_class)->dispose(object);
}

static void finalize(GObject *object) {
	KonepureopticalEventhandlerPrivate *priv = KONEPUREOPTICAL_EVENTHANDLER(object)->priv;
	...
	G_OBJECT_CLASS(konepureoptical_eventhandler_parent_class)->finalize(object);
}

static void konepureoptical_eventhandler_class_init(KonepureopticalEventhandlerClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	gobject_class->dispose = dispose;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KonepureopticalEventhandlerPrivate));
}

ROCCAT_EVENTHANDLER_PLUGIN_API RoccatEventhandlerPlugin *roccat_eventhandler_plugin_new(void) {
	return ROCCAT_EVENTHANDLER_PLUGIN(g_object_new(KONEPUREOPTICAL_EVENTHANDLER_TYPE, NULL));
}
