...
#include "konepuremilitary_dbus_server.h"
...
GType konepuremilitary_eventhandler_get_type(void);

#define KONEPUREMILITARY_EVENTHANDLER_TYPE (konepuremilitary_eventhandler_get_type())
#define KONEPUREMILITARY_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), KONEPUREMILITARY_EVENTHANDLER_TYPE, KonepuremilitaryEventhandler))
#define IS_KONEPUREMILITARY_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), KONEPUREMILITARY_EVENTHANDLER_TYPE))
#define KONEPUREMILITARY_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREMILITARY_EVENTHANDLER_TYPE, KonepuremilitaryEventhandlerClass))
#define IS_KONEPUREMILITARY_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREMILITARY_EVENTHANDLER_TYPE))
#define KONEPUREMILITARY_EVENTHANDLER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREMILITARY_EVENTHANDLER_TYPE, KonepuremilitaryEventhandlerPrivate))

typedef struct _KonepuremilitaryEventhandler KonepuremilitaryEventhandler;
typedef struct _KonepuremilitaryEventhandlerClass KonepuremilitaryEventhandlerClass;
typedef struct _KonepuremilitaryEventhandlerPrivate KonepuremilitaryEventhandlerPrivate;

struct _KonepuremilitaryEventhandler {
	GObject parent;
	KonepuremilitaryEventhandlerPrivate *priv;
};

struct _KonepuremilitaryEventhandlerClass {
	GObjectClass parent_class;
};

struct _KonepuremilitaryEventhandlerPrivate {
	RoccatEventhandlerHost *host;
	KonepuremilitaryDBusServer *dbus_server;
	...
	KoneplusGfx *gfx;
	KonepuremilitaryEventhandlerChannel *channel;
...
static void profile_changed(KonepuremilitaryEventhandler *eventhandler, guint profile_number) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	...
	konepuremilitary_dbus_server_emit_profile_changed(priv->dbus_server, profile_number);
}

static gboolean set_profile(KonepuremilitaryEventhandler *eventhandler, guint profile_number) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static void window_changed_cb(RoccatEventhandlerHost *host, gchar const *title, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static gboolean talk_easyshift(KonepuremilitaryEventhandler *eventhandler, gboolean state) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
...
static gboolean talk_easyshift_lock(KonepuremilitaryEventhandler *eventhandler, gboolean state) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talk_easyaim(KonepuremilitaryEventhandler *eventhandler, guchar state) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_set_led_rgb(KonepuremilitaryEventhandler *eventhandler, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_restore_led_rgb(KonepuremilitaryEventhandler *eventhandler) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static guint konepuremilitary_eventhandler_get_talk_device(RoccatEventhandlerPlugin *self) {
	KonepuremilitaryEventhandlerPrivate *priv = KONEPUREMILITARY_EVENTHANDLER(self)->priv;
...
static void konepuremilitary_eventhandler_talk_easyshift(RoccatEventhandlerPlugin *self, gboolean state) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	talk_easyshift(eventhandler, state);
}

static void konepuremilitary_eventhandler_talk_easyshift_lock(RoccatEventhandlerPlugin *self, gboolean state) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	talk_easyshift_lock(eventhandler, state);
}

static void konepuremilitary_eventhandler_talk_easyaim(RoccatEventhandlerPlugin *self, guchar state) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	talk_easyaim(eventhandler, state);
}

static void konepuremilitary_eventhandler_talkfx_set_led_rgb(RoccatEventhandlerPlugin *self, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void konepuremilitary_eventhandler_talkfx_restore_led_rgb(RoccatEventhandlerPlugin *self) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	talkfx_restore_led_rgb(eventhandler);
}

static void talk_easyshift_cb(KonepuremilitaryDBusServer *server, guchar state, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	talk_easyshift(eventhandler, state);
}

static void talk_easyshift_lock_cb(KonepuremilitaryDBusServer *server, guchar state, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	talk_easyshift_lock(eventhandler, state);
}

static void talk_easyaim_cb(KonepuremilitaryDBusServer *server, guchar state, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	talk_easyaim(eventhandler, state);
}

static void talkfx_set_led_rgb_cb(KonepuremilitaryDBusServer *server, guint effect, guint ambient_color, guint event_color, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void talkfx_restore_led_rgb_cb(KonepuremilitaryDBusServer *server, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	talkfx_restore_led_rgb(eventhandler);
}

static void gfx_set_led_rgb_cb(KonepuremilitaryDBusServer *server, guint index, guint color, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	koneplus_gfx_set_color(eventhandler->priv->gfx, index, color);
}

static void gfx_get_led_rgb_cb(KonepuremilitaryDBusServer *server, guint index, guint *color, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	*color = koneplus_gfx_get_color(eventhandler->priv->gfx, index);
}

static void gfx_update_cb(KonepuremilitaryDBusServer *server, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
...
static void open_gui_cb(gpointer source, gpointer user_data) {
	gchar *path;
	path = g_build_path("/", INSTALL_PREFIX, "bin", "roccatkonepuremilitaryconfig", (gchar const *)NULL);
	roccat_double_fork(path);
	g_free(path);
}

static void profile_data_changed_outside_cb(KonepuremilitaryDBusServer *server, guchar number, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->rmp[profile_index] = konepuremilitary_rmp_load_actual(profile_index);
}

static void configuration_reload(KonepuremilitaryEventhandler *eventhandler) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	if (priv->config)
...
static void configuration_changed_outside_cb(KonepuremilitaryDBusServer *server, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	configuration_reload(eventhandler);
}

static void cpi_changed_cb(KonepuremilitaryEventhandlerChannel *channel, guchar cpi_index, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	guint value = konepuremilitary_cpi_rpm_to_real(koneplus_rmp_get_cpi_level_x(priv->rmp[priv->actual_profile_index], cpi_index));
...
static void quicklaunch_cb(KonepuremilitaryEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
...
static void sensitivity_changed_cb(KonepuremilitaryEventhandlerChannel *channel, guchar x, guchar y, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
...
static void talk_cb(KonepuremilitaryEventhandlerChannel *channel, guchar key_index, guchar event, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_start_cb(KonepuremilitaryEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_stop_cb(KonepuremilitaryEventhandlerChannel *channel, gpointer user_data) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(user_data);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
...
static gboolean profile_data_load(KonepuremilitaryEventhandler *eventhandler, GError **error) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	guint i;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		priv->rmp[i] = konepuremilitary_rmp_load(priv->device, i, error);
		if (priv->rmp[i] == NULL)
			return FALSE;
	}
	return TRUE;
...
static void device_add_cb(RoccatDeviceScannerInterface *interface, RoccatDevice *device, gpointer user_data) {
	...
	if (!priv->profile_note)
		priv->profile_note = roccat_notification_profile_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREMILITARY_DEVICE_NAME);
	if (!priv->cpi_note)
		priv->cpi_note = roccat_notification_cpi_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREMILITARY_DEVICE_NAME);
	if (!priv->sensitivity_note)
		priv->sensitivity_note = roccat_notification_sensitivity_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREMILITARY_DEVICE_NAME);
	if (!priv->timer_note)
		priv->timer_note = roccat_notification_timer_new(roccat_eventhandler_host_get_notificator(priv->host), KONEPUREMILITARY_DEVICE_NAME);
	...
	priv->dbus_server = konepuremilitary_dbus_server_new();
	...
	konepuremilitary_dbus_server_connect(priv->dbus_server);
	...
	konepuremilitary_eventhandler_channel_start(priv->channel, priv->device, &local_error);
	if (local_error) {
		g_warning(_("Could not start hidraw eventhandler: %s"), local_error->message);
		g_clear_error(&local_error);
	}
...
static void stop(KonepuremilitaryEventhandler *eventhandler) {
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	guint i;
	...
	konepuremilitary_eventhandler_channel_stop(priv->channel);
...
static void konepuremilitary_eventhandler_start(RoccatEventhandlerPlugin *self, RoccatEventhandlerHost *host) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->channel = konepuremilitary_eventhandler_channel_new();
	...
	priv->device_scanner = konepuremilitary_device_scanner_new();
...
static void konepuremilitary_eventhandler_stop(RoccatEventhandlerPlugin *self) {
	KonepuremilitaryEventhandler *eventhandler = KONEPUREMILITARY_EVENTHANDLER(self);
	KonepuremilitaryEventhandlerPrivate *priv = eventhandler->priv;
...
static void konepuremilitary_roccat_eventhandler_interface_init(RoccatEventhandlerPluginInterface *iface) {
	iface->start = konepuremilitary_eventhandler_start;
	iface->stop = konepuremilitary_eventhandler_stop;
	iface->get_talk_device = konepuremilitary_eventhandler_get_talk_device;
	iface->talk_easyshift = konepuremilitary_eventhandler_talk_easyshift;
	iface->talk_easyshift_lock = konepuremilitary_eventhandler_talk_easyshift_lock;
	iface->talk_easyaim = konepuremilitary_eventhandler_talk_easyaim;
	iface->talkfx_set_led_rgb = konepuremilitary_eventhandler_talkfx_set_led_rgb;
	iface->talkfx_restore_led_rgb = konepuremilitary_eventhandler_talkfx_restore_led_rgb;
...
G_DEFINE_TYPE_WITH_CODE(KonepuremilitaryEventhandler, konepuremilitary_eventhandler, G_TYPE_OBJECT,
		G_IMPLEMENT_INTERFACE(ROCCAT_EVENTHANDLER_PLUGIN_TYPE, konepuremilitary_roccat_eventhandler_interface_init));

static void konepuremilitary_eventhandler_init(KonepuremilitaryEventhandler *eventhandler) {
	eventhandler->priv = KONEPUREMILITARY_EVENTHANDLER_GET_PRIVATE(eventhandler);
}

static void dispose(GObject *object) {
	konepuremilitary_eventhandler_stop(ROCCAT_EVENTHANDLER_PLUGIN(object));

	G_OBJECT_CLASS(konepuremilitary_eventhandler_parent_class)->dispose(object);
}

static void finalize(GObject *object) {
	KonepuremilitaryEventhandlerPrivate *priv = KONEPUREMILITARY_EVENTHANDLER(object)->priv;
	...
	G_OBJECT_CLASS(konepuremilitary_eventhandler_parent_class)->finalize(object);
}

static void konepuremilitary_eventhandler_class_init(KonepuremilitaryEventhandlerClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	gobject_class->dispose = dispose;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KonepuremilitaryEventhandlerPrivate));
}

ROCCAT_EVENTHANDLER_PLUGIN_API RoccatEventhandlerPlugin *roccat_eventhandler_plugin_new(void) {
	return ROCCAT_EVENTHANDLER_PLUGIN(g_object_new(KONEPUREMILITARY_EVENTHANDLER_TYPE, NULL));
}
