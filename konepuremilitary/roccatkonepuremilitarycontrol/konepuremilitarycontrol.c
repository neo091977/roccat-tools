...
static GOptionContext *commandline_parse(int *argc, char ***argv) {
	...
	string = g_strdup_printf(_("- controls extended capabilities of Roccat %s mice"), KONEPUREMILITARY_DEVICE_NAME_COMBINED);
...
static gboolean reset(RoccatDevice *konepure, GError **error) {
	...
	proxy = konepuremilitary_dbus_proxy_new();
	if (!proxy)
		return TRUE;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i)
		konepuremilitary_dbus_emit_profile_data_changed_outside(proxy, i + 1);
...
int main(int argc, char **argv) {
	...
	device = konepuremilitary_device_first();
	if (device == NULL) {
		g_critical(_("No %s found."), KONEPUREMILITARY_DEVICE_NAME_COMBINED);
		retval = EXIT_FAILURE;
		goto exit1;
	}

	if (parameter_just_activate_driver_state) {
		if (!koneplus_device_state_write(device, KONEPLUS_DEVICE_STATE_STATE_ON, &local_error)) {
	...
	if (parameter_just_deactivate_driver_state) {
		if (!koneplus_device_state_write(device, KONEPLUS_DEVICE_STATE_STATE_OFF, &local_error)) {
	...
	if (parameter_just_update_firmware) {
		if (!roccat_firmware_update_console(device, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_firmware,
				KONEPUREMILITARY_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT,
	...
	if (parameter_just_update_srom) {
		if (!roccat_srom_update_console(device, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_srom)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
	...
	if (parameter_sensor_read) {
		if (!print_sensor_value(device, &local_error)) {
	...
	if (parameter_sensor_write != -1) {
		koneplus_sensor_write_register(device, parameter_sensor_register, parameter_sensor_write, &local_error);
		if (local_error) {
	...
	if (parameter_load != -1) {
		rmp = konepuremilitary_rmp_load(device, parameter_load - 1, &local_error);
		if (!rmp) {
		...
		rmp = koneplus_rmp_read_with_path(parameter_in_rmp, konepuremilitary_rmp_defaults(), &local_error);
		if (!rmp) {
	...
	if (parameter_save != -1) {
		if (!konepuremilitary_rmp_save(device, rmp, parameter_save - 1, &local_error)) {
			g_critical(_("Could not save profile %i: %s"), parameter_save, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		konepuremilitary_dbus_emit_profile_data_changed_outside_instant(parameter_save);
	}
	...
	if (parameter_activate_profile != -1) {
		if (!koneplus_actual_profile_write(device, parameter_activate_profile -1, &local_error)) {
			g_critical(_("Could not activate profile %i: %s"), parameter_activate_profile, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		konepuremilitary_dbus_emit_profile_changed_outside_instant(parameter_activate_profile);
	}
