...
#define KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_TYPE, KonepuremilitaryconfigConfigurationDialogClass))
#define IS_KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_TYPE))
#define KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_TYPE, KonepuremilitaryconfigConfigurationDialogPrivate))

typedef struct _KonepuremilitaryconfigConfigurationDialogClass KonepuremilitaryconfigConfigurationDialogClass;

struct _KonepuremilitaryconfigConfigurationDialogClass {
	GtkDialogClass parent_class;
};

struct _KonepuremilitaryconfigConfigurationDialogPrivate {
...
G_DEFINE_TYPE(KonepuremilitaryconfigConfigurationDialog, konepuremilitaryconfig_configuration_dialog, GTK_TYPE_DIALOG);

static GtkWidget *notification_frame_new(KonepuremilitaryconfigConfigurationDialog *konepuremilitaryconfig_configuration_dialog) {
	KonepuremilitaryconfigConfigurationDialogPrivate *priv = konepuremilitaryconfig_configuration_dialog->priv;
...
static GtkWidget *general_frame_new(KonepuremilitaryconfigConfigurationDialog *konepuremilitaryconfig_configuration_dialog) {
	KonepuremilitaryconfigConfigurationDialogPrivate *priv = konepuremilitaryconfig_configuration_dialog->priv;
...
gboolean konepuremilitaryconfig_configuration_dialog(GtkWindow *parent, RoccatKeyFile *config) {
	KonepuremilitaryconfigConfigurationDialog *dialog;
	gboolean retval = TRUE;

	dialog = KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG(konepuremilitaryconfig_configuration_dialog_new(parent));

	konepuremilitaryconfig_configuration_dialog_set_configuration(dialog, config);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		konepuremilitaryconfig_configuration_dialog_get_configuration(dialog, config);
		if (!konepuremilitaryconfig_configuration_save(parent, config))
			retval = FALSE;
	}
...
GtkWidget *konepuremilitaryconfig_configuration_dialog_new(GtkWindow *parent) {
	KonepuremilitaryconfigConfigurationDialog *konepuremilitaryconfig_configuration_dialog;

	konepuremilitaryconfig_configuration_dialog = g_object_new(KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_TYPE, NULL);

	gtk_dialog_add_buttons(GTK_DIALOG(konepuremilitaryconfig_configuration_dialog),
			GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
			GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			NULL);
	gtk_window_set_title(GTK_WINDOW(konepuremilitaryconfig_configuration_dialog), _("Edit preferences"));
	gtk_window_set_transient_for(GTK_WINDOW(konepuremilitaryconfig_configuration_dialog), parent);
	gtk_window_set_modal(GTK_WINDOW(konepuremilitaryconfig_configuration_dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(konepuremilitaryconfig_configuration_dialog), TRUE);
	gtk_window_set_type_hint(GTK_WINDOW(konepuremilitaryconfig_configuration_dialog), GDK_WINDOW_TYPE_HINT_NORMAL);

	return GTK_WIDGET(konepuremilitaryconfig_configuration_dialog);
}

static void konepuremilitaryconfig_configuration_dialog_init(KonepuremilitaryconfigConfigurationDialog *konepuremilitaryconfig_configuration_dialog) {
	KonepuremilitaryconfigConfigurationDialogPrivate *priv = KONEPUREMILITARYCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(konepuremilitaryconfig_configuration_dialog);
	GtkVBox *content_area;

	konepuremilitaryconfig_configuration_dialog->priv = priv;

	content_area = GTK_VBOX(gtk_dialog_get_content_area(GTK_DIALOG(konepuremilitaryconfig_configuration_dialog)));

	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(konepuremilitaryconfig_configuration_dialog), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(content_area), general_frame_new(konepuremilitaryconfig_configuration_dialog), TRUE, TRUE, 0);

	gtk_widget_show_all(GTK_WIDGET(content_area));
}

static void konepuremilitaryconfig_configuration_dialog_class_init(KonepuremilitaryconfigConfigurationDialogClass *klass) {
	g_type_class_add_private(klass, sizeof(KonepuremilitaryconfigConfigurationDialogPrivate));
}

gboolean konepuremilitaryconfig_configuration_save(GtkWindow *parent, RoccatKeyFile *config) {
	GError *error = NULL;

	konepuremilitary_configuration_save(config, &error);
