...
static gboolean parse_profile_number_parameter(gint number, GError **error) {
	if (number < 1 || number > SKELTR_PROFILE_NUM) {
...
static GOptionContext *commandline_parse(int *argc, char ***argv) {
	...
	string = g_strdup_printf(_("- controls extended capabilities of Roccat %s keyboards"), SKELTR_DEVICE_NAME);
...
static gboolean print_firmware(RoccatDevice *device, GError **error) {
	...
	firmware_version = skeltr_firmware_version_read(device, error);
	if (*error)
		return FALSE;
...
static gboolean reset(RoccatDevice *device, GError **error) {
	...
	if (!skeltr_reset(device, SKELTR_RESET_FUNCTION_RESET, error))
		return FALSE;

	proxy = skeltr_dbus_proxy_new();
	if (!proxy)
		return TRUE;

	for (i = 0; i < SKELTR_PROFILE_NUM; ++i)
		skeltr_dbus_emit_profile_data_changed_outside(proxy, i + 1);
...
int main(int argc, char **argv) {
	...
	SkeltrProfileData *profile_data = NULL;
	SkeltrProfile *profile = NULL;
	...
	device = skeltr_device_first();
	if (device == NULL) {
		g_critical(_("No %s found."), SKELTR_DEVICE_NAME);
		retval = EXIT_FAILURE;
		goto exit1;
	}
	...
	if (parameter_just_update_firmware) {
		if (!roccat_firmware_update_console(device, SKELTR_INTERFACE_KEYBOARD, parameter_just_update_firmware,
				SKELTR_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT,
				SKELTR_FIRMWARE_UPDATE_WAIT_PREPARE, SKELTR_FIRMWARE_UPDATE_WAIT_INIT_DATA, SKELTR_FIRMWARE_UPDATE_WAIT_DATA, SKELTR_FIRMWARE_UPDATE_FINAL_WRITE_CHECK)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
	...
	if (parameter_load != -1) {
		if (!skeltr_profile_data_load(profile_data, parameter_load - 1, &local_error)) {
		...
		if (!skeltr_rkp_to_profile_data(profile_data, rkp, length)) {
	...
	if (parameter_save != -1) {
		...
		skeltr_dbus_emit_profile_data_changed_outside_instant(parameter_save);
	}
	...
	if (parameter_default_profile != -1) {
		configuration = skeltr_configuration_load();
		skeltr_configuration_set_default_profile_number(configuration, parameter_default_profile);
		...
		roccat_configuration_free(configuration);
		skeltr_dbus_emit_configuration_changed_outside_instant();
	}
	...
	if (parameter_just_print_actual_profile || parameter_activate_profile != -1) {
		...
		if (parameter_activate_profile != -1) {
			...
			if (!skeltr_profile_write(device, profile, &local_error)) {
				g_critical(_("Could not activate profile %i: %s"), parameter_activate_profile, local_error->message);
				retval = EXIT_FAILURE;
				goto exit4;
			}
			skeltr_dbus_emit_profile_changed_outside_instant(parameter_activate_profile);
		}
	}
