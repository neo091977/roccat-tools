...
static gboolean post_parse_func(GOptionContext *context, GOptionGroup *group, gpointer data, GError **error) {
	...
	if (just_counter > 1) {
		g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_FAILED, _("Just give one of -v, --actual-profile, --reset, --driver-off, --driver-on, --firmware, --srom"));
		return FALSE;
	}
...
static gboolean print_actual_profile(RoccatDevice *konextd, GError **error) {
	guint profile_index = koneplus_actual_profile_read(konextd, error);
	if (*error)
		return FALSE;
	g_print("%i\n", profile_index + 1);
	return TRUE;
...
static gboolean print_firmware(RoccatDevice *konextd, GError **error) {
	gchar *firmware_string;
	guint firmware_version;

	firmware_version = koneplus_firmware_version_read(konextd, error);
	if (*error)
		return FALSE;
...
static gboolean print_sensor_value(RoccatDevice *konextd, GError **error) {
	guint8 value;
	value = koneplus_sensor_read_register(konextd, parameter_sensor_register, error);
	if (*error)
		return FALSE;
	g_print("0x%02x\n", value);
	return TRUE;
...
static gboolean reset(RoccatDevice *konextd, GError **error) {
	DBusGProxy *proxy;
	guint i;

	if (!koneplus_reset(konextd, error))
		return FALSE;
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i)
		konextd_dbus_emit_profile_data_changed_outside(proxy, i + 1);
...
int main(int argc, char **argv) {
	...
	konextd = konextd_device_first();
	if (konextd == NULL) {
		g_critical(_("No %s found."), KONEXTD_DEVICE_NAME);
		retval = EXIT_FAILURE;
		goto exit1;
	}

	if (parameter_just_activate_driver_state) {
		if (!koneplus_device_state_write(konextd, KONEPLUS_DEVICE_STATE_STATE_ON, &local_error)) {
	...
	if (parameter_just_deactivate_driver_state) {
		if (!koneplus_device_state_write(konextd, KONEPLUS_DEVICE_STATE_STATE_OFF, &local_error)) {
	...
	if (parameter_just_update_firmware) {
		if (!roccat_firmware_update_console(konextd, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_firmware,
				KONEPLUS_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT,
				ROCCAT_FIRMWARE_UPDATE_WAIT_PREPARE_DEFAULT, ROCCAT_FIRMWARE_UPDATE_WAIT_INIT_DATA_DEFAULT, ROCCAT_FIRMWARE_UPDATE_WAIT_DATA_DEFAULT, ROCCAT_FIRMWARE_UPDATE_FINAL_WRITE_CHECK_DEFAULT)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
	...
	if (parameter_just_update_srom) {
		if (!roccat_srom_update_console(konextd, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_srom)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
	...
	if (parameter_just_print_actual_profile) {
		if (!print_actual_profile(konextd, &local_error)) {
	...
	if (parameter_read_firmware) {
		if (!print_firmware(konextd, &local_error)) {
	...
	if (parameter_sensor_read) {
		if (!print_sensor_value(konextd, &local_error)) {
	...
	if (parameter_sensor_write != -1) {
		koneplus_sensor_write_register(konextd, parameter_sensor_register, parameter_sensor_write, &local_error);
		if (local_error) {
	...
	if (parameter_load != -1) {
		rmp = konextd_rmp_load(konextd, parameter_load - 1, &local_error);
		if (!rmp) {
		...
		rmp = koneplus_rmp_read_with_path(parameter_in_rmp, konextd_rmp_defaults(), &local_error);
		if (!rmp) {
	...
	if (parameter_save != -1) {
		if (!konextd_rmp_save(konextd, rmp, parameter_save - 1, &local_error)) {
			g_critical(_("Could not save profile %i: %s"), parameter_save, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		konextd_dbus_emit_profile_data_changed_outside_instant(parameter_save);
	}
	...
	if (parameter_activate_profile != -1) {
		if (!koneplus_actual_profile_write(konextd, parameter_activate_profile -1, &local_error)) {
			g_critical(_("Could not activate profile %i: %s"), parameter_activate_profile, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		konextd_dbus_emit_profile_changed_outside_instant(parameter_activate_profile);
	}

exit3:
	koneplus_rmp_free(rmp);
exit2:
	g_object_unref(G_OBJECT(konextd));
exit1:
