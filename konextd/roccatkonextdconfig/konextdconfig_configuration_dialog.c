...
#define KONEXTDCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEXTDCONFIG_CONFIGURATION_DIALOG_TYPE, KonextdconfigConfigurationDialogClass))
#define IS_KONEXTDCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEXTDCONFIG_CONFIGURATION_DIALOG_TYPE))
#define KONEXTDCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEXTDCONFIG_CONFIGURATION_DIALOG_TYPE, KonextdconfigConfigurationDialogPrivate))

typedef struct _KonextdconfigConfigurationDialogClass KonextdconfigConfigurationDialogClass;

struct _KonextdconfigConfigurationDialogClass {
	GtkDialogClass parent_class;
};

struct _KonextdconfigConfigurationDialogPrivate {
...
G_DEFINE_TYPE(KonextdconfigConfigurationDialog, konextdconfig_configuration_dialog, GTK_TYPE_DIALOG);

static GtkWidget *notification_frame_new(KonextdconfigConfigurationDialog *konextdconfig_configuration_dialog) {
	KonextdconfigConfigurationDialogPrivate *priv = konextdconfig_configuration_dialog->priv;
...
static GtkWidget *general_frame_new(KonextdconfigConfigurationDialog *konextdconfig_configuration_dialog) {
	KonextdconfigConfigurationDialogPrivate *priv = konextdconfig_configuration_dialog->priv;
...
gboolean konextdconfig_configuration_dialog(GtkWindow *parent, RoccatKeyFile *config) {
	KonextdconfigConfigurationDialog *dialog;
	gboolean retval = TRUE;

	dialog = KONEXTDCONFIG_CONFIGURATION_DIALOG(konextdconfig_configuration_dialog_new(parent));

	konextdconfig_configuration_dialog_set_configuration(dialog, config);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		konextdconfig_configuration_dialog_get_configuration(dialog, config);
		if (!konextdconfig_configuration_save(parent, config))
			retval = FALSE;
	}
...
GtkWidget *konextdconfig_configuration_dialog_new(GtkWindow *parent) {
	KonextdconfigConfigurationDialog *konextdconfig_configuration_dialog;

	konextdconfig_configuration_dialog = g_object_new(KONEXTDCONFIG_CONFIGURATION_DIALOG_TYPE, NULL);

	gtk_dialog_add_buttons(GTK_DIALOG(konextdconfig_configuration_dialog),
			GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
			GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			NULL);
	gtk_window_set_title(GTK_WINDOW(konextdconfig_configuration_dialog), _("Edit preferences"));
	gtk_window_set_transient_for(GTK_WINDOW(konextdconfig_configuration_dialog), parent);
	gtk_window_set_modal(GTK_WINDOW(konextdconfig_configuration_dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(konextdconfig_configuration_dialog), TRUE);
	gtk_window_set_type_hint(GTK_WINDOW(konextdconfig_configuration_dialog), GDK_WINDOW_TYPE_HINT_NORMAL);

	return GTK_WIDGET(konextdconfig_configuration_dialog);
}

static void konextdconfig_configuration_dialog_init(KonextdconfigConfigurationDialog *konextdconfig_configuration_dialog) {
	KonextdconfigConfigurationDialogPrivate *priv = KONEXTDCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(konextdconfig_configuration_dialog);
	GtkVBox *content_area;

	konextdconfig_configuration_dialog->priv = priv;

	content_area = GTK_VBOX(gtk_dialog_get_content_area(GTK_DIALOG(konextdconfig_configuration_dialog)));

	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(konextdconfig_configuration_dialog), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(content_area), general_frame_new(konextdconfig_configuration_dialog), TRUE, TRUE, 0);

	gtk_widget_show_all(GTK_WIDGET(content_area));
}

static void konextdconfig_configuration_dialog_class_init(KonextdconfigConfigurationDialogClass *klass) {
	g_type_class_add_private(klass, sizeof(KonextdconfigConfigurationDialogPrivate));
}

gboolean konextdconfig_configuration_save(GtkWindow *parent, RoccatKeyFile *config) {
	GError *error = NULL;

	konextd_configuration_save(config, &error);
