...
static gchar const * const group_name = "KiroGUI";
...
static gchar *configuration_path(void) {
	...
	gchar *path = g_build_filename(dir, "kiro.ini", NULL);
	g_free(dir);
	return path;
}

RoccatKeyFile *kiro_configuration_load(void) {
...
void kiro_configuration_free(RoccatKeyFile *config) {
	if (config == NULL)
		return;
...
gboolean kiro_configuration_save(RoccatKeyFile *config, GError **error) {
	if (config == NULL)
		return TRUE;
...
gchar *kiro_configuration_get_rmp_path(RoccatKeyFile *config) {
	return roccat_key_file_get_string_with_default(config, group_name, rmp_path_key, g_get_home_dir());
}

void kiro_configuration_set_rmp_path(RoccatKeyFile *config, gchar const *path) {
	roccat_key_file_set_string(config, group_name, rmp_path_key, path);
}

gint kiro_configuration_get_default_profile_number(RoccatKeyFile *config) {
	return roccat_key_file_get_integer_with_default(config, group_name, default_profile_number_key, 0);
}

void kiro_configuration_set_default_profile_number(RoccatKeyFile *config, gint new_value) {
