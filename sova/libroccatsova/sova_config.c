...
static gchar const * const group_name = "SovaGUI";
...
static gchar *configuration_path(void) {
	...
	gchar *path = g_build_filename(dir, "sova.ini", NULL);
	g_free(dir);
	return path;
}

RoccatKeyFile *sova_configuration_load(void) {
...
gchar *sova_configuration_get_rkp_path(RoccatKeyFile *config) {
	return roccat_key_file_get_string_with_default(config, group_name, rkp_path_key, g_get_home_dir());
}

void sova_configuration_set_rkp_path(RoccatKeyFile *config, gchar const *path) {
	roccat_key_file_set_string(config, group_name, rkp_path_key, path);
}

gint sova_configuration_get_default_profile_number(RoccatKeyFile *config) {
	return roccat_key_file_get_integer_with_default(config, group_name, default_profile_number_key, 0);
}

void sova_configuration_set_default_profile_number(RoccatKeyFile *config, gint new_value) {
	roccat_key_file_set_integer(config, group_name, default_profile_number_key, new_value);
}

gchar *sova_configuration_get_layout(RoccatKeyFile *config) {
	return roccat_key_file_get_string_with_default(config, group_name, layout_key, "");
}

void sova_configuration_set_layout(RoccatKeyFile *config, gchar const *layout) {
