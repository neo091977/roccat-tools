...
#include "ryosmk_dbus_plain_services.h"
...
DBusGProxy *ryos_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(
			...
			RYOS_DBUS_SERVER_PATH,
			RYOS_DBUS_SERVER_INTERFACE);
...
gboolean ryos_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Ryosmk_profile_data_changed_outside, profile_number);
}
...
gboolean ryos_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = ryos_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean ryos_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Ryosmk_profile_changed_outside, profile_number);
}
...
gboolean ryos_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = ryos_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean ryos_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Ryosmk_configuration_changed_outside);
}
...
gboolean ryos_dbus_emit_configuration_changed_outside_instant(void) {
	...
	retval = ryos_dbus_emit_configuration_changed_outside(proxy);
	dbus_roccat_proxy_free(proxy);
	return retval;
}
