...
static GOptionContext *commandline_parse(int *argc, char ***argv) {
	...
	string = g_strdup_printf(_("- controls extended capabilities of Roccat %s mice"), KONEXTDOPTICAL_DEVICE_NAME);
...
static gboolean print_actual_profile(RoccatDevice *konextdoptical, GError **error) {
	guint profile_index = koneplus_actual_profile_read(konextdoptical, error);
	if (*error)
		return FALSE;
	g_print("%i\n", profile_index + 1);
	return TRUE;
...
static gboolean print_firmware(RoccatDevice *konextdoptical, GError **error) {
	gchar *firmware_string;
	guint firmware_version;

	firmware_version = koneplus_firmware_version_read(konextdoptical, error);
	if (*error)
		return FALSE;
...
static gboolean print_sensor_value(RoccatDevice *konextdoptical, GError **error) {
	guint8 value;
	value = koneplus_sensor_read_register(konextdoptical, parameter_sensor_register, error);
	if (*error)
		return FALSE;
	g_print("0x%02x\n", value);
	return TRUE;
...
static gboolean reset(RoccatDevice *konextdoptical, GError **error) {
	DBusGProxy *proxy;
	guint i;

	if (!koneplus_reset(konextdoptical, error))
		return FALSE;

	proxy = konextdoptical_dbus_proxy_new();
	if (!proxy)
		return TRUE;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i)
		konextdoptical_dbus_emit_profile_data_changed_outside(proxy, i + 1);
...
int main(int argc, char **argv) {
	...
	if (parameter_just_update_firmware) {
		if (!roccat_firmware_update_console(konextdoptical, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_firmware,
	...
	if (parameter_just_update_srom) {
		if (!roccat_srom_update_console(konextdoptical, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_srom)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
