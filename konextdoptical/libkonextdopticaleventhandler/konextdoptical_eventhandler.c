...
#include "konextdoptical_dbus_server.h"
...
GType konextdoptical_eventhandler_get_type(void);

#define KONEXTDOPTICAL_EVENTHANDLER_TYPE (konextdoptical_eventhandler_get_type())
#define KONEXTDOPTICAL_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), KONEXTDOPTICAL_EVENTHANDLER_TYPE, KonextdopticalEventhandler))
#define IS_KONEXTDOPTICAL_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), KONEXTDOPTICAL_EVENTHANDLER_TYPE))
#define KONEXTDOPTICAL_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEXTDOPTICAL_EVENTHANDLER_TYPE, KonextdopticalEventhandlerClass))
#define IS_KONEXTDOPTICAL_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEXTDOPTICAL_EVENTHANDLER_TYPE))
#define KONEXTDOPTICAL_EVENTHANDLER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEXTDOPTICAL_EVENTHANDLER_TYPE, KonextdopticalEventhandlerPrivate))

typedef struct _KonextdopticalEventhandler KonextdopticalEventhandler;
typedef struct _KonextdopticalEventhandlerClass KonextdopticalEventhandlerClass;
typedef struct _KonextdopticalEventhandlerPrivate KonextdopticalEventhandlerPrivate;

struct _KonextdopticalEventhandler {
	GObject parent;
	KonextdopticalEventhandlerPrivate *priv;
};

struct _KonextdopticalEventhandlerClass {
	GObjectClass parent_class;
};

struct _KonextdopticalEventhandlerPrivate {
	RoccatEventhandlerHost *host;
	KonextdopticalDBusServer *dbus_server;
	...
	KoneplusGfx *gfx;
	KonextdopticalEventhandlerChannel *channel;
...
static void profile_changed(KonextdopticalEventhandler *eventhandler, guint profile_number) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	konextdoptical_dbus_server_emit_profile_changed(priv->dbus_server, profile_number);
}

static gboolean set_profile(KonextdopticalEventhandler *eventhandler, guint profile_number) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static void window_changed_cb(RoccatEventhandlerHost *host, gchar const *title, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static gboolean talk_easyshift(KonextdopticalEventhandler *eventhandler, gboolean state) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
...
static gboolean talk_easyshift_lock(KonextdopticalEventhandler *eventhandler, gboolean state) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talk_easyaim(KonextdopticalEventhandler *eventhandler, guchar state) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_set_led_rgb(KonextdopticalEventhandler *eventhandler, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talkfx_restore_led_rgb(KonextdopticalEventhandler *eventhandler) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static guint konextdoptical_eventhandler_get_talk_device(RoccatEventhandlerPlugin *self) {
	KonextdopticalEventhandlerPrivate *priv = KONEXTDOPTICAL_EVENTHANDLER(self)->priv;
...
static void konextdoptical_eventhandler_talk_easyshift(RoccatEventhandlerPlugin *self, gboolean state) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	talk_easyshift(eventhandler, state);
}

static void konextdoptical_eventhandler_talk_easyshift_lock(RoccatEventhandlerPlugin *self, gboolean state) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	talk_easyshift_lock(eventhandler, state);
}

static void konextdoptical_eventhandler_talk_easyaim(RoccatEventhandlerPlugin *self, guchar state) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	talk_easyaim(eventhandler, state);
}

static void konextdoptical_eventhandler_talkfx_set_led_rgb(RoccatEventhandlerPlugin *self, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void konextdoptical_eventhandler_talkfx_restore_led_rgb(RoccatEventhandlerPlugin *self) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	talkfx_restore_led_rgb(eventhandler);
}

static void talk_easyshift_cb(KonextdopticalDBusServer *server, guchar state, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	talk_easyshift(eventhandler, state);
}

static void talk_easyshift_lock_cb(KonextdopticalDBusServer *server, guchar state, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	talk_easyshift_lock(eventhandler, state);
}

static void talk_easyaim_cb(KonextdopticalDBusServer *server, guchar state, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	talk_easyaim(eventhandler, state);
}

static void talkfx_set_led_rgb_cb(KonextdopticalDBusServer *server, guint effect, guint ambient_color, guint event_color, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void talkfx_restore_led_rgb_cb(KonextdopticalDBusServer *server, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	talkfx_restore_led_rgb(eventhandler);
}

static void gfx_set_led_rgb_cb(KonextdopticalDBusServer *server, guint index, guint color, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	koneplus_gfx_set_color(eventhandler->priv->gfx, index, color);
}

static void gfx_get_led_rgb_cb(KonextdopticalDBusServer *server, guint index, guint *color, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	*color = koneplus_gfx_get_color(eventhandler->priv->gfx, index);
}

static void gfx_update_cb(KonextdopticalDBusServer *server, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
...
static void open_gui_cb(gpointer source, gpointer user_data) {
	gchar *path;
	path = g_build_path("/", INSTALL_PREFIX, "bin", "roccatkonextdopticalconfig", (gchar const *)NULL);
	roccat_double_fork(path);
	g_free(path);
}

static void profile_data_changed_outside_cb(KonextdopticalDBusServer *server, guchar number, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->rmp[profile_index] = konextdoptical_rmp_load_actual(profile_index);
}

static void configuration_reload(KonextdopticalEventhandler *eventhandler) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	if (priv->config)
...
static void configuration_changed_outside_cb(KonextdopticalDBusServer *server, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	configuration_reload(eventhandler);
}

static void cpi_changed_cb(KonextdopticalEventhandlerChannel *channel, guchar cpi_index, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	roccat_notification_cpi_update(priv->cpi_note,
			...
			koneplus_configuration_get_notification_volume(priv->config),
			koneplus_rmp_get_cpi_level_x(priv->rmp[priv->actual_profile_index], cpi_index) * 100,
			koneplus_rmp_get_cpi_level_y(priv->rmp[priv->actual_profile_index], cpi_index) * 100);
}
...
static void quicklaunch_cb(KonextdopticalEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void sensitivity_changed_cb(KonextdopticalEventhandlerChannel *channel, guchar x, guchar y, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void talk_cb(KonextdopticalEventhandlerChannel *channel, guchar key_index, guchar event, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_start_cb(KonextdopticalEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_stop_cb(KonextdopticalEventhandlerChannel *channel, gpointer user_data) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(user_data);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static gboolean profile_data_load(KonextdopticalEventhandler *eventhandler, GError **error) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint i;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		priv->rmp[i] = konextdoptical_rmp_load(priv->device, i, error);
		if (priv->rmp[i] == NULL)
			return FALSE;
	}
	return TRUE;
...
static void device_add_cb(RoccatDeviceScannerInterface *interface, RoccatDevice *device, gpointer user_data) {
	...
	if (!priv->profile_note)
		priv->profile_note = roccat_notification_profile_new(roccat_eventhandler_host_get_notificator(priv->host), KONEXTDOPTICAL_DEVICE_NAME);
	if (!priv->cpi_note)
		priv->cpi_note = roccat_notification_cpi_new(roccat_eventhandler_host_get_notificator(priv->host), KONEXTDOPTICAL_DEVICE_NAME);
	if (!priv->sensitivity_note)
		priv->sensitivity_note = roccat_notification_sensitivity_new(roccat_eventhandler_host_get_notificator(priv->host), KONEXTDOPTICAL_DEVICE_NAME);
	if (!priv->timer_note)
		priv->timer_note = roccat_notification_timer_new(roccat_eventhandler_host_get_notificator(priv->host), KONEXTDOPTICAL_DEVICE_NAME);
	...
	priv->dbus_server = konextdoptical_dbus_server_new();
	...
	konextdoptical_dbus_server_connect(priv->dbus_server);
	...
	konextdoptical_eventhandler_channel_start(priv->channel, priv->device, &local_error);
	if (local_error) {
...
static void stop(KonextdopticalEventhandler *eventhandler) {
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	guint i;
	...
	konextdoptical_eventhandler_channel_stop(priv->channel);
...
static void konextdoptical_eventhandler_start(RoccatEventhandlerPlugin *self, RoccatEventhandlerHost *host) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->channel = konextdoptical_eventhandler_channel_new();
	...
	priv->device_scanner = konextdoptical_device_scanner_new();
...
static void konextdoptical_eventhandler_stop(RoccatEventhandlerPlugin *self) {
	KonextdopticalEventhandler *eventhandler = KONEXTDOPTICAL_EVENTHANDLER(self);
	KonextdopticalEventhandlerPrivate *priv = eventhandler->priv;
...
static void konextdoptical_roccat_eventhandler_interface_init(RoccatEventhandlerPluginInterface *iface) {
	iface->start = konextdoptical_eventhandler_start;
	iface->stop = konextdoptical_eventhandler_stop;
	iface->get_talk_device = konextdoptical_eventhandler_get_talk_device;
	iface->talk_easyshift = konextdoptical_eventhandler_talk_easyshift;
	iface->talk_easyshift_lock = konextdoptical_eventhandler_talk_easyshift_lock;
	iface->talk_easyaim = konextdoptical_eventhandler_talk_easyaim;
	iface->talkfx_set_led_rgb = konextdoptical_eventhandler_talkfx_set_led_rgb;
	iface->talkfx_restore_led_rgb = konextdoptical_eventhandler_talkfx_restore_led_rgb;
...
G_DEFINE_TYPE_WITH_CODE(KonextdopticalEventhandler, konextdoptical_eventhandler, G_TYPE_OBJECT,
		G_IMPLEMENT_INTERFACE(ROCCAT_EVENTHANDLER_PLUGIN_TYPE, konextdoptical_roccat_eventhandler_interface_init));

static void konextdoptical_eventhandler_init(KonextdopticalEventhandler *eventhandler) {
	eventhandler->priv = KONEXTDOPTICAL_EVENTHANDLER_GET_PRIVATE(eventhandler);
}

static void dispose(GObject *object) {
	konextdoptical_eventhandler_stop(ROCCAT_EVENTHANDLER_PLUGIN(object));

	G_OBJECT_CLASS(konextdoptical_eventhandler_parent_class)->dispose(object);
}

static void finalize(GObject *object) {
	KonextdopticalEventhandlerPrivate *priv = KONEXTDOPTICAL_EVENTHANDLER(object)->priv;
	...
	G_OBJECT_CLASS(konextdoptical_eventhandler_parent_class)->finalize(object);
}

static void konextdoptical_eventhandler_class_init(KonextdopticalEventhandlerClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	gobject_class->dispose = dispose;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KonextdopticalEventhandlerPrivate));
}

ROCCAT_EVENTHANDLER_PLUGIN_API RoccatEventhandlerPlugin *roccat_eventhandler_plugin_new(void) {
	return ROCCAT_EVENTHANDLER_PLUGIN(g_object_new(KONEXTDOPTICAL_EVENTHANDLER_TYPE, NULL));
}
