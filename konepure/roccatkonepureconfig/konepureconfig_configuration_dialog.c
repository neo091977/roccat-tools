...
#define KONEPURECONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPURECONFIG_CONFIGURATION_DIALOG_TYPE, KonepureconfigConfigurationDialogClass))
#define IS_KONEPURECONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPURECONFIG_CONFIGURATION_DIALOG_TYPE))
#define KONEPURECONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPURECONFIG_CONFIGURATION_DIALOG_TYPE, KonepureconfigConfigurationDialogPrivate))

typedef struct _KonepureconfigConfigurationDialogClass KonepureconfigConfigurationDialogClass;

struct _KonepureconfigConfigurationDialogClass {
	GtkDialogClass parent_class;
};

struct _KonepureconfigConfigurationDialogPrivate {
...
G_DEFINE_TYPE(KonepureconfigConfigurationDialog, konepureconfig_configuration_dialog, GTK_TYPE_DIALOG);

static GtkWidget *notification_frame_new(KonepureconfigConfigurationDialog *konepureconfig_configuration_dialog) {
	KonepureconfigConfigurationDialogPrivate *priv = konepureconfig_configuration_dialog->priv;
...
static GtkWidget *general_frame_new(KonepureconfigConfigurationDialog *konepureconfig_configuration_dialog) {
	KonepureconfigConfigurationDialogPrivate *priv = konepureconfig_configuration_dialog->priv;
...
gboolean konepureconfig_configuration_dialog(GtkWindow *parent, RoccatKeyFile *config) {
	KonepureconfigConfigurationDialog *dialog;
	gboolean retval = TRUE;

	dialog = KONEPURECONFIG_CONFIGURATION_DIALOG(konepureconfig_configuration_dialog_new(parent));

	konepureconfig_configuration_dialog_set_configuration(dialog, config);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		konepureconfig_configuration_dialog_get_configuration(dialog, config);
		if (!konepureconfig_configuration_save(parent, config))
			retval = FALSE;
	}
...
GtkWidget *konepureconfig_configuration_dialog_new(GtkWindow *parent) {
	KonepureconfigConfigurationDialog *konepureconfig_configuration_dialog;

	konepureconfig_configuration_dialog = g_object_new(KONEPURECONFIG_CONFIGURATION_DIALOG_TYPE, NULL);

	gtk_dialog_add_buttons(GTK_DIALOG(konepureconfig_configuration_dialog),
			GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
			GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			NULL);
	gtk_window_set_title(GTK_WINDOW(konepureconfig_configuration_dialog), _("Edit preferences"));
	gtk_window_set_transient_for(GTK_WINDOW(konepureconfig_configuration_dialog), parent);
	gtk_window_set_modal(GTK_WINDOW(konepureconfig_configuration_dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(konepureconfig_configuration_dialog), TRUE);
	gtk_window_set_type_hint(GTK_WINDOW(konepureconfig_configuration_dialog), GDK_WINDOW_TYPE_HINT_NORMAL);

	return GTK_WIDGET(konepureconfig_configuration_dialog);
}

static void konepureconfig_configuration_dialog_init(KonepureconfigConfigurationDialog *konepureconfig_configuration_dialog) {
	KonepureconfigConfigurationDialogPrivate *priv = KONEPURECONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(konepureconfig_configuration_dialog);
	GtkVBox *content_area;

	konepureconfig_configuration_dialog->priv = priv;

	content_area = GTK_VBOX(gtk_dialog_get_content_area(GTK_DIALOG(konepureconfig_configuration_dialog)));

	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(konepureconfig_configuration_dialog), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(content_area), general_frame_new(konepureconfig_configuration_dialog), TRUE, TRUE, 0);

	gtk_widget_show_all(GTK_WIDGET(content_area));
}

static void konepureconfig_configuration_dialog_class_init(KonepureconfigConfigurationDialogClass *klass) {
	g_type_class_add_private(klass, sizeof(KonepureconfigConfigurationDialogPrivate));
}

gboolean konepureconfig_configuration_save(GtkWindow *parent, RoccatKeyFile *config) {
	GError *error = NULL;

	konepure_configuration_save(config, &error);
