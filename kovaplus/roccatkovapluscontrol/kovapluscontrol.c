...
static gboolean parse_profile_number_parameter(gint number, GError **error) {
	if (number < 1 || number > KOVAPLUS_PROFILE_NUM) {
...
static GOptionContext *commandline_parse(int *argc, char ***argv) {
	...
	string = g_strdup_printf(_("- controls extended capabilities of Roccat %s mice"), KOVAPLUS_DEVICE_NAME);
...
static gboolean print_actual_profile(RoccatDevice *kovaplus, GError **error) {
	guint profile_index = kovaplus_actual_profile_read(kovaplus, error);
	if (*error)
		return FALSE;
	g_print("%i\n", profile_index + 1);
	return TRUE;
...
static gboolean print_firmware(RoccatDevice *kovaplus, GError **error) {
	guint firmware_version;
	gchar *firmware_string;

	firmware_version = kovaplus_firmware_version_read(kovaplus, error);
	if (*error)
		return FALSE;
...
static gboolean profile_activate(RoccatDevice *kovaplus, guint profile_number, GError **error) {
	return kovaplus_actual_profile_write(kovaplus, profile_number - 1, error);
}

static gboolean reset(RoccatDevice *kovaplus, GError **error) {
	DBusGProxy *proxy;
	guint i;

	if (!kovaplus_reset(kovaplus, error))
		return FALSE;

	proxy = kovaplus_dbus_proxy_new();
	if (!proxy)
		return TRUE;

	for (i = 0; i < KOVAPLUS_PROFILE_NUM; ++i)
		kovaplus_dbus_emit_profile_data_changed_outside(proxy, i + 1);
